...
amp::ManipulatorState MyLinkManipulator2D::getConfigurationFromIK(const Eigen::Vector2d& end_effector_location) const {
    //Currently only works for 2&3-link robots
    //PRINT_VEC2("Finding Config for ", end_effector_location);
    int n = m_link_lengths.size();
    amp::ManipulatorState config(n);
    int ind0, ind1;
    if (n == 2) {
        ind0 = 0;
        ind1 = 1;
    ...
    Eigen::Vector2d newGoal = end_effector_location;
    if (n == 3) {
        ...
        newGoal = Utils::rotateVec(end_effector_location - link1Loc, -theta1 * 180 / M_PI);
        config(0) = theta1;
    }
    ...
    // run basic 2-link IK
    double c2, s2, c3, s3;
    c3 = ((pow(newGoal(0),2) + pow(newGoal(1),2)) - (pow(m_link_lengths[ind0],2) + pow(m_link_lengths[ind1],2)));
    //DEBUG("c3 is " << c3);
    c3 /= (2 * m_link_lengths[ind0] * m_link_lengths[ind1]);

    s3 = sqrt(1 - pow(c3, 2));

    c2 = newGoal(0) * (m_link_lengths[ind0] + m_link_lengths[ind1] * c3) + newGoal(1) * m_link_lengths[ind1] * s3;
    c2 /= (pow(newGoal(0),2) + pow(newGoal(1),2));

    s2 = newGoal(1) * (m_link_lengths[ind0] + m_link_lengths[ind1] * c3) - newGoal(0) * m_link_lengths[ind1] * s3;
